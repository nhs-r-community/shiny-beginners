...
- Learn to create a shiny app from scratch
- Different _input controls_ we can use in the `ui`
- Outputs for displaying reactive content in the `ui`
...
::: callout-note
## app.R vs ui.R + server.R
You might occasionally see the `ui` and `server` objects split into two different scripts.
This was the traditional way, however, `app.R` is more common now.
Either is fine
:::
